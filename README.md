# BAT Monitor Dashboard / BAT 監控儀表板
...
To let the dashboard capture output, run the target process directly inside the BAT file:
...
That launches a separate CMD window, so the dashboard cannot capture its output.
...
This refreshes the PNG files under `docs/images/`.
